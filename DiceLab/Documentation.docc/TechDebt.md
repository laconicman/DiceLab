...
| TD-6 | RealityKit physics tuning was redone on simulator data (`-impulseLog`: 2.2 s settle, impulses ≤0.17 N·s) — device feel still unconfirmed | Roll could still feel off on hardware; `maxImpulse` was calibrated from sim contacts | Device run: sanity-check `Toss`/`Rest`/`maxImpulse` in `RealityTableController` |
...
| TD-8 | Play volume is wider than the camera frames — dice settling near ±x/±z edges sit half-off-screen | Cosmetic: valid rolls read fine, but a die can rest half-hidden | Narrow the bounds toward the camera's footprint, or widen FOV/raise the camera |
| TD-9 | `RealityTableController: DiceTable` conformance warns about crossing into main-actor-isolated code | Becomes a hard error under Swift 6 language mode | When adopting Swift 6: `@MainActor` on the protocol (requires `DiceTableController` isolation work) or `nonisolated` conformance members |
